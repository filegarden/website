 ...
 *
 * If a redirect should occur, returns `true` server-side, or throws silently
 * client-side since the caller won't need to continue after redirecting to
 * another page. If no redirect should occur, returns `false`.
 *
 * Note that the redirect is client-side so the user's sign-in status isn't
